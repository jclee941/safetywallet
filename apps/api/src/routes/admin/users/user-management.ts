...
const logger = createLogger("admin/users/user-management");
...
// GET /users
userManagementRouter.get("/users", requireAdmin, async (c) => {
...
userManagementRouter.get("/users/restrictions", requireAdmin, async (c) => {
  const db = drizzle(c.env.DB);
  const limit = Math.min(parseInt(c.req.query("limit") || "50"), 100);
  const offset = parseInt(c.req.query("offset") || "0");
