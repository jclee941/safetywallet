...
app.get("/users", requireExportAccess, exportRateLimit, async (c) => {
  ...
  const page =
    typeof parsed.data.page === "string"
      ? parseInt(parsed.data.page, 10)
      : parsed.data.page;
...
app.get("/posts", requireExportAccess, exportRateLimit, async (c) => {
  ...
  try {
    ...
    logger.info("Post export completed", {
      userId: auth.user.id,
    ...
    logger.error("Post export failed", {
      error: e instanceof Error ? e.message : String(e),
    });
    return error(
      c,
...
app.get("/attendance", requireExportAccess, exportRateLimit, async (c) => {
  ...
  return csvResponse(
    c,
    csv,
    `attendance_${formatYearMonth(new Date())}_p1.csv`,
  );
});
